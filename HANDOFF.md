...
| Reasoning on the local sidecar | `llama-server --help` from the studio's own runtime lists `--reasoning on\|off\|auto`, `--reasoning-budget N` and `--reasoning-format`. The sidecar is now started with them, driven by the same setting the cloud path sends as `reasoning.effort`, and with `--reasoning-format deepseek` so thinking lands in `reasoning_content` where the reader looks. |
| Switching between installed sets | `POST /setup/select` refuses an incomplete set (`"a runnable MiniMax Music3 installation requires exactly one depth component"`) and accepts a complete installed one. Before this, the only way to change quantisation was to download something. |
| Settings is not the first run | The Models section of Settings shows "Models and quantisations", no first-run badge and no hardware banner; the first-run page still shows both. Screenshotted in English and Russian from the running build. |
| Cover prompt templates | `POST /v1/cover-templates/render` filled `Album cover for "{title}". Style: {style}. Scene: {excerpt}. Length {duration}.` from a Cyrillic track into `Album cover for "Неон". Style: dream pop. Scene: Неон дрожит над мокрым городом и я иду домой. Length 0:00.` Unknown placeholders are left as typed. |
| External links in the packaged application | The cause was the empty scope: `tauri-plugin-opener`'s `open_url` returns `ForbiddenUrl` unless the capability lists the URL, and `opener:allow-open-url` alone lists nothing. `capabilities/default.json` now allows `https://*` and `http://*`, and the generated `gen/schemas/capabilities.json` carries the scope into the binary. |
| Screenshots | 18, taken with the studio running against a live engine on this machine: create, library and studio tools in all five languages, plus settings and models in English and Russian. |
| Tests | `cargo test --workspace` — 81 passing (auto-title and cover-prompt rendering added). `npm --prefix app run build` and `tsc --noEmit` clean. |
